...
# # Use custom encoder module using openai api key and model from env
...
faiss_index_path = "data/faiss.index"
if not Path(faiss_index_path).exists():
    # Create a dummy index or raise clearer error if strictly required
    print("[WARN] FAISS index not found. Using empty index for safety.")
    # Initialize an empty index with correct dimension (assuming 768 for text-embedding-004)
    index = faiss.IndexFlatL2(768) 
else:
    index = faiss.read_index(faiss_index_path)

# Load documents aligned with FAISS vectors
kb_path = "data/kb_texts.jsonl"
documents = []
if Path(kb_path).exists():
    with open(kb_path, "r", encoding="utf-8") as f:
        documents = [json.loads(line.strip()) if line.strip() else {} for line in f.readlines()]

# This was overwriting your Gemini client previously!
client = OpenAI(api_key=os.environ.get("OPENAI_API_KEY"))
...
def retrieve_documents(query: str, k: int = 5):
    ...
    # Note: Ensure q_emb is 2D array (1, dimension)
    q_emb = embedding_model.encode([query]).astype("float32")

    # 2. Safety Check: Index Existence
    if index is None or index.ntotal == 0:
        print("[WARN] FAISS index is empty or not loaded.")
        return []

    # 3. CRITICAL FIX: Check dimensions before search to prevent crash
    # index.d is the dimension of the vectors stored in FAISS
    # q_emb.shape[1] is the dimension of your generated query embedding
    if index.d != q_emb.shape[1]:
        error_msg = (
            f"Dimension Mismatch! Index expects {index.d} dims, "
            f"but Model produced {q_emb.shape[1]} dims.\n"
            f"ACTION REQUIRED: Delete 'data/faiss.index' and rebuild your knowledge base."
        )
        print(f"[ERROR] {error_msg}")
        # Raise specific error so API returns 500 with helpful message
        raise ValueError(error_msg)

    # 4. Search FAISS
    # Ensure k doesn't exceed total docs
    search_k = min(k, index.ntotal)
    scores, indices = index.search(q_emb, search_k)
    
    # 5. Retrieve documents
    # Validate indices to prevent crashes if index is out of sync with documents list
